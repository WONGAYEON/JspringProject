...
<p><br/></p>
  		...
  		<tr>
  	 	...
  	 	<tr>
  	  	<td colspan="2" class="text-end">
  	  		<c:if test="${pag > 1}">
  	  			<a href="guestList?pag=1" title="첫페이지" class="text-decoration-none text-dark link-primary link-underline-opacity-0 link-underline-opacity-100-hover">◁◁</a>
  	  			<a href="guestList?pag=${pag-1}" title="이전페이지" class="text-decoration-none text-dark link-primary link-underline-opacity-0 link-underline-opacity-100-hover">◀</a>
  	  		</c:if>
			 	 ${pag} / ${totPage}
  	  		<c:if test="${pag < totPage}">
  	  			<a href="guestList?pag=${pag+1}" title="다음페이지" class="text-decoration-none text-dark link-primary link-underline-opacity-0 link-underline-opacity-100-hover">▶</a>
  	  			<a href="guestList?pag=${totPage}" title="마지막페이지" class="text-decoration-none text-dark link-primary link-underline-opacity-0 link-underline-opacity-100-hover">▷▷</a>
  	  		</c:if>
  	  	</td>
  		</tr>
  	</table>
 ...
 <div class="text-center">
 		...
 		<c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize)+3}" varStatus="st">
