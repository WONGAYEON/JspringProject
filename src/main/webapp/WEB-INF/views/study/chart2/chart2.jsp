  ...
  <script>
    ...
    function chartShow(part) {
    	if(part == 'barV') {
    		...
    		location.href = 'googleChart2Recently?part=line';
    	}
    }
  </script>
</head>
  ...
  <div>
    <p>학습할 차트 선택하세요..
      <select name="part" id="part" onchange="chartChange()">
        ...
        <option value="line" ${vo.part == 'line' ? 'selected' : ''}>최근방문자수</option>
        <option value="bar" ${part == 'barChartVisitCount2' ? 'selected' : ''}>많이방문한회원7명</option>
      </select>
    </p>
    <hr/>${visitDates}=====
    <div id="demo"></div>
    <div>
      ...
      <c:if test="${vo.part == 'line'}"><jsp:include page="line.jsp" /></c:if>
      <%-- <c:if test="${part == 'barChartVisitCount2'}"><jsp:include page="barChartVisitCount.jsp" /></c:if> --%>
    </div>
    <hr/>
  </div>
</div>
